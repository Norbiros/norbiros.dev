...
<template>
  <UMain class="mt-8">
    <div
      v-if="page"
      class="blog-post-shell min-h-screen"
    >
      <nav
      ...
      <article
        class="post-article relative mx-auto w-full max-w-[70ch] text-lg xl:mx-0 xl:max-w-none"
      >
        <ULink
          to="/blog"
        ...
        <header class="mt-8 flex flex-col gap-5">
          ...
          <h1 class="text-4xl leading-none font-bold tracking-tight text-balance text-highlighted sm:text-5xl">
            {{ page.title }}
          </h1>
          <p class="text-xl leading-snug tracking-tight text-muted text-pretty sm:text-2xl">
            {{ page.description }}
          </p>
        ...
        <div class="article-prose text-lg leading-8 text-pretty antialiased">
          <ContentRenderer
            v-if="page.body"
            :value="page"
          />
        ...
        <div class="mt-10 flex items-center justify-end gap-2 text-sm text-muted">
          <UButton
            v-if="rawMarkdown"
            size="sm"
            variant="link"
